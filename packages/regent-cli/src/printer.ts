...
export function renderUsageScreen(configPath: string): string {
  const lines = [
    ...
    ...renderUsageGroup("Techtree Next Commands", [
      ...
      "regent techtree node lineage list <id>",
      "regent techtree node lineage claim <id> --input @file.json",
      "regent techtree node lineage withdraw <id> --claim-id <claim-id>",
      "regent techtree node cross-chain-links list <id>",
      "regent techtree node cross-chain-links create <id> --input @file.json",
      "regent techtree node cross-chain-links clear <id>",
      "regent techtree node create ... [--cross-chain-link @file.json] [--paid-payload @file.json]",
      "regent techtree comment add --node-id <id> --body-markdown ...",
      "regent techtree autoskill init skill [path]",
      "regent techtree autoskill notebook pair [path]",
      "regent techtree autoskill publish skill [path]",
      "regent techtree autoskill publish eval [path]",
      "regent techtree autoskill publish result [path] --skill-node-id ... --eval-node-id ...",
      "regent techtree autoskill review --kind community|replicable --skill-node-id ...",
      "regent techtree autoskill listing create --skill-node-id ... --price-usdc ...",
      "regent techtree autoskill buy <node-id>",
      "regent techtree autoskill pull <node-id> [path]",
    ...
    ...renderUsageGroup("BBH Next Loop", [
      ...
      "regent techtree bbh genome score [path]",
      "regent techtree bbh genome improve [path]",
      "regent techtree bbh genome propose <capsule-id> [path]",
      "regent techtree reviewer orcid link",
      "regent techtree review list --kind certification",
      "regent techtree certificate verify <capsule-id>",
    ...
    ...renderUsageGroup("Messaging + Adjacent Work", [
      ...
      "regent xmtp init",
      "regent xmtp status",
      "regent xmtp group permissions <conversation-id>",
      "regent xmtp group update-permission <conversation-id> --type add-member --policy admin",
      "regent xmtp group add-admin <conversation-id> --address <wallet>",
      "regent xmtp doctor",
      "regent autolaunch ...",
      "regent autolaunch safe wizard",
      "regent autolaunch safe create",
