...
const SEPOLIA_RPC_ENV = "ETH_SEPOLIA_RPC_URL";
...
const requireSepoliaRpcUrl = (args: ParsedCliArgs): string => {
  const explicit = normalizeText(getFlag(args, "rpc-url"));
  const envValue = normalizeText(process.env[SEPOLIA_RPC_ENV]);
  const resolved = explicit ?? envValue;
  if (!resolved) {
    throw new Error(
      `Sepolia RPC URL is required. Pass --rpc-url <url> or set ${SEPOLIA_RPC_ENV}.`,
    );
  }
...
export async function runAutolaunchSafeCreate(
  ...
  const rpcUrl = requireSepoliaRpcUrl(args);
