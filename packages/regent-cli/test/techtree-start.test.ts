...
describe("techtree start wizard", () => {
  ...
  it("stops and asks for Sepolia RPC before minting a missing identity", async () => {
    ...
    expect(output.stdout).toContain("ETH_SEPOLIA_RPC_URL");
  ...
  it("reuses an already authenticated identity without logging in again", async () => {
    ...
    startWizardDeps.authStatus = vi.fn().mockResolvedValue({
      authenticated: true,
      session: { walletAddress: "0xabc", chainId: 11155111, nonce: "1", keyId: "k", receipt: "r", receiptExpiresAt: "2099-01-01T00:00:00.000Z", audience: "techtree" },
      agentIdentity: {
        walletAddress: "0xabc",
        chainId: 11155111,
        registryAddress: TEST_REGISTRY,
