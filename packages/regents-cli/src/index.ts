...
export async function runCliEntrypoint(rawArgs: string[]): Promise<number> {
  try {
    ...
    if (namespace === "auth" && subcommand === "login") {
      await runAuthSiwaLogin(parsedArgs, configPath);
      return 0;
    }

    if (namespace === "auth" && subcommand === "status") {
      await runAuthSiwaStatus(configPath);
      return 0;
    }

    if (namespace === "auth" && subcommand === "logout") {
    ...
    if (namespace === "identity" && subcommand === "status") {
      return await runIdentityStatus(parsedArgs, configPath);
    }

    if (namespace === "wallet" && subcommand === "status") {
      return await runWalletStatus(parsedArgs, configPath);
    }

    if (namespace === "wallet" && subcommand === "setup") {
      return await runWalletSetup(parsedArgs, configPath);
    }

    if (namespace === "mcp" && subcommand === "export" && maybeThird === "hermes") {
      return await runMcpExportHermes(parsedArgs);
    }
