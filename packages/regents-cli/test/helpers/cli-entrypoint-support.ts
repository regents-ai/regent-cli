  ...
  if (method.startsWith("techtree.v1.")) {
    ...
    if (method === "techtree.v1.bbh.run.exec") {
      ...
      return {
        ...
        capsule: {
          ...
          execution_defaults: {
            solver: {
              kind: split === "climb" ? "hermes" : "skydiscover",
              entrypoint: split === "climb" ? "hermes" : "uv run techtree-bbh sky-search",
              search_algorithm: split === "climb" ? null : "best_of_n",
            },
            evaluator: {
              kind: "hypotest",
              dataset_ref: "provider/capsule_test",
              benchmark_ref: "capsule_test",
    ...
    if (method === "techtree.v1.bbh.run.solve") {
      return {
        ...
        workspace_path: workspacePath,
        run_id: "run_test",
        solver: String(payload.solver ?? "hermes"),
        produced_files: [
          "analysis.py",
          "final_answer.md",
          "outputs/verdict.json",
  ...
  if (method === "techtree.autoskill.notebook.pair") {
    ...
    return {
      ...
      instructions: {
        ...
        next_regent_commands: [`regents techtree autoskill publish skill ${workspacePath}`],
      },
    };
  }
